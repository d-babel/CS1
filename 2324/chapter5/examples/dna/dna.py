...
def find_repeated_strings(source, substring):
    ...
    while index != -1:
        count = 1
        while source.startswith(substring):
            source = source[len(substring):]  # Move past the current match
            count += 1
        max_count = max(max_count, count)
        source = source[index + len(substring):]
        index = source.find(substring)
    return max_count
...
def main():
    ...
    print("File, STR, Original Method Time (s), Sliding Window Method Time (s), Harvard Solution Time (s), New Method Time (s)")
