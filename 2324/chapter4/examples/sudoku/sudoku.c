...
bool startup(void)
{
    ...
    if (has_colors())
    {
        ...
        if (init_pair(PAIR_ORIGINAL_NUM, FG_ORIGINAL_NUM, BG_DIGITS) == ERR ||
        init_pair(PAIR_USER_NUM, FG_DIGITS, BG_DIGITS) == ERR ||
        init_pair(PAIR_BANNER, FG_BANNER, BG_BANNER) == ERR ||
